...
use Churnsolution\Stripe\StripeObject;
...
abstract class Util
{
    ...
    {
        $types = \Churnsolution\Stripe\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \Churnsolution\Stripe\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \Churnsolution\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
