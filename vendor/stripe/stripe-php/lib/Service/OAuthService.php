...
class OAuthService extends \Churnsolution\Stripe\Service\AbstractService
{
    /**
     ...
     *
     * @return \Churnsolution\Stripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \Churnsolution\Stripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \Churnsolution\Stripe\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \Churnsolution\Stripe\StripeClient([clientId => <CLIENT-ID>
     ...
     *
     * @return \Churnsolution\Stripe\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \Churnsolution\Stripe\Util\RequestOptions::parse($opts);
    }
