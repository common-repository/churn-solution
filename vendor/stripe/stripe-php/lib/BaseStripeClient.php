...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    const DEFAULT_CONFIG = [
        ...
        'client_id' => null,
        'stripe_account' => null,
        'stripe_version' => \Churnsolution\Stripe\Util\ApiVersion::CURRENT,
    ...
    /** @var \Churnsolution\Stripe\Util\RequestOptions */
    private $defaultOpts;
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \Churnsolution\Stripe\Util\RequestOptions::parse([
    ...
    public function request($method, $path, $params, $opts)
    {
        ...
        $requestor = new \Churnsolution\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl, $this->getAppInfo());
     ...
     * @param array|\Churnsolution\Stripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \Churnsolution\Stripe\Collection of ApiResources
     */
    public function requestCollection($method, $path, $params, $opts)
    {
        $obj = $this->request($method, $path, $params, $opts);
        if (!($obj instanceof \Churnsolution\Stripe\Collection)) {
            $received_class = \get_class($obj);
            $msg = "Expected to receive `Stripe\\Collection` object from Stripe API. Instead received `{$received_class}`.";

            throw new \Churnsolution\Stripe\Exception\UnexpectedValueException($msg);
        }
    ...
    {
        $obj = $this->request($method, $path, $params, $opts);
        if (!($obj instanceof \Churnsolution\Stripe\SearchResult)) {
            $received_class = \get_class($obj);
    ...
    /**
     * @param \Churnsolution\Stripe\Util\RequestOptions $opts
     *
     * @throws \Churnsolution\Stripe\Exception\AuthenticationException
     *
     * @return string
     */
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \Churnsolution\Stripe\Exception\AuthenticationException($msg);
        }
     ...
     *
     * @throws \Churnsolution\Stripe\Exception\InvalidArgumentException
     */
    private function validateConfig($config)
    {
        // api_key
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }

        if (null !== $config['api_key'] && ('' === $config['api_key'])) {
            $msg = 'api_key cannot be the empty string';

            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException($msg);
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('files_base must be a string');
        }

        // app info
        if (null !== $config['app_info'] && !\is_array($config['app_info'])) {
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('app_info must be an array');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \Churnsolution\Stripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
}
